...
class MediaKitPlayer extends AudioPlayerPlatform {
  ...
  bool _nativeOutputConfigured = false;
  bool _usingWasapiExclusive = false;
    ...
    _streamSubscriptions = [
      ...
      _player.stream.error.listen((error) {
        if (_usingWasapiExclusive &&
          ...
          unawaited(_fallbackToWasapiShared(reason: error));
        }
  ...
  bool _looksLikeExclusiveModeError(String message) {
  ...
  Future<void> _logNativeAudioState(String stage) async {
    try {
      final ao = _normalizePropertyValue(await getProperty(_player, 'ao'));
      final exclusive = _normalizePropertyValue(
        await getProperty(_player, 'audio-exclusive'),
      );
      final device = _normalizePropertyValue(
        await getProperty(_player, 'audio-device'),
      );
      _emitNativeAudioRouteLog(
        'stage=$stage, ao=$ao, audio-exclusive=$exclusive, '
        'audio-device=$device',
      );
    } catch (error) {
      _emitNativeAudioRouteLog(
          'stage=$stage, failed to query native output: $error');
    }
  }

  String _normalizePropertyValue(String? value) {
  ...
  Future<void> _configureNativeOutput() async {
    if (_nativeOutputConfigured) {
      _emitNativeAudioRouteLog(
        'configured output -> reuse existing native output config',
      );
      await _logNativeAudioState('reuse-config');
      return;
    }
    _nativeOutputConfigured = true;

    if (JustAudioMediaKit.prefetchPlaylist) {
      await setProperty(_player, 'prefetch-playlist', 'yes');
    }

    for (final entry in JustAudioMediaKit.nativeMpvProperties.entries) {
      await setProperty(_player, entry.key, entry.value);
    }
    ...
    // Enforce WASAPI path on Windows as requested by project spec.
    await setProperty(_player, 'ao', 'wasapi');
    ...
    if (JustAudioMediaKit.preferWasapiExclusive) {
      try {
        ...
        _logger.warning(
          'Failed to enable WASAPI exclusive ($error). Falling back to shared.',
        );
        if (JustAudioMediaKit.fallbackToWasapiShared) {
          await _fallbackToWasapiShared(reason: '$error');
        }
      }
    } else {
      await setProperty(_player, 'audio-exclusive', 'no');
  ...
  Future<void> _fallbackToWasapiShared({required String reason}) async {
    try {
      ...
      _usingWasapiExclusive = false;
      _emitNativeAudioRouteLog(
        'exclusive fallback -> shared. reason: $reason',
      );
      await _logNativeAudioState('post-fallback');
    } catch (error) {
      _logger.severe('Failed to fallback to WASAPI shared mode: $error');
    }
  }
  ...
  Future<LoadResponse> load(LoadRequest request) async {
    ...
    if (request.audioSourceMessage is ConcatenatingAudioSourceMessage) {
      ...
      playable = _convertAudioSourceIntoMediaKit(request.audioSourceMessage);
    ...
    try {
      ...
      if (_usingWasapiExclusive && JustAudioMediaKit.fallbackToWasapiShared) {
        await _fallbackToWasapiShared(reason: '$error');
        await _player.open(playable, play: _playing);
      } else {
        rethrow;
