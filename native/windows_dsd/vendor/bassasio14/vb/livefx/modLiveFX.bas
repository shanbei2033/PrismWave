...
Static Function Initialize() As Boolean
    ...
    fxchan = BASS_StreamCreate(BASS_ASIO_GetRate(), 2, BASS_SAMPLE_FLOAT Or BASS_STREAM_DECODE, STREAMPROC_PUSH, 0)

    'enable ASIO channels (using same stream for input and output)
    Call BASS_ASIO_ChannelEnableBASS(1, 0, fxchan, BASSTRUE)
    Call BASS_ASIO_ChannelEnableBASS(0, 0, fxchan, BASSTRUE)
